...
$content_width = 585;
/*
 *Enqueueing some additional scripts
 * */
wp_enqueue_script ( 'jquerytools', 'http://cdn.jquerytools.org/1.2.5/all/jquery.tools.min.js', array('jquery'), '1.0');
wp_enqueue_style ( 'jquerytools', WP_CONTENT_URL . '/themes/edegree/css/jquerytools.css', false, '1.0', 'screen');
...
/*
 * some additional scripting by me
 *
 * */


/*
 * registering menu places
 * */
 ...
 * filtering content
 * */
add_filter('the_content', 'tt_filter_content');

function tt_filter_content($content)
{

	$exprs = array('/\[last-post([\s]+)(.+)\]/', "/\[category='(.+)'\]/"); // to find
	foreach ($exprs as $expr)
	{
		...
		if (preg_match_all($expr, $content, $params,PREG_PATTERN_ORDER))
		{
			...
			foreach ($params[1] as $param)
			{
				...
				{
					case '/\[last-post([\s]+)(.+)\]/':
						$content = str_replace("[last-post $param]",tt_last_posts($param,1,true),$content);
						break;
					case "/\[category='(.+)'\]/":
						$content = str_replace("[category='$param']",tt_category_posts($param,1,true),$content);
						break;
				}
			}
		}
...
function tt_last_posts($cat,$num = 10,$content = false)
{
$args = array(
//'numberposts' => $num,
'orderby' => 'date',
...
return tt_posts($args);
}
/*shows posts in a category*/
function tt_category_posts($cat,$num = 10,$content = false){
$args = array(
//'numberposts' => $num,
'orderby' => 'date',
'order' => 'DESC',
...
?>
    ...
    <?php if(isset($args['paged']) and $args['paged']==1):
$_res = '
    <div class="navigation">
        <p class="alignleft">' . previous_posts_link('&laquo; Previous Page') . '</p>
        <p class="alignright">' . next_posts_link('Next Page &raquo;') . '</p>
        <div class="recover"></div>
    </div>
    ';?>
    <?php endif;
...
<div id="post-' . $post->ID . '" class="post">
    <div class="entry_header">
        <p class="entry-date">' . get_the_time('M y',$post->ID) . '<br />
        <span class="date">' . get_the_time('j',$post->ID) . '</span></p>
';
        is_home() ? $_res .= '<h2 class="home">' : $_res .= '<h1>';
$_res .= '
        <a href="' . get_permalink($post->ID) . '">' . $post->post_title . '</a>
';
//$_res .= edit_post_link('Edit', '<span class="editpost">', '</span>');
        is_home() ? $_res .= '</h2>' : $_res .= '</h1>';

/*        if ($post->comment_status != 'closed' && !post_password_required()):
$_res .= '
        <div class="comment-bubble"><span>' .
         comments_popup_link('<span class="nocomment">No Comment</span>', '&nbsp;1 Comment', '% Comments') .
        '</span></div>';
        endif;*/
...
';
$content = get_the_content(FALSE, FALSE, FALSE);
//$content = $post->post_content;
$content = apply_filters('the_content', $content);
$content = str_replace(']]>', ']]&gt;', $content);
$_res .= $content;
        /*get_the_content('<span class="readmore">Read the rest of '. get_the_title('', '', false). ' &raquo;</span>', FALSE) .
...
/*
 * function for showing featured news
 * */

function tt_featured_gallery(){
/* WTZ
